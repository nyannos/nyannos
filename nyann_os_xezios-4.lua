...
do
  ...
  for _, url in ipairs(urls) do
    local ok, res = pcall(function()
      ...
      -- Force Code font in library
      src = src:gsub("Enum%.Font%.GothamBold", "Enum.Font.Code")
      src = src:gsub("Enum%.Font%.GothamMedium", "Enum.Font.Code")
      src = src:gsub("Enum%.Font%.GothamSemibold", "Enum.Font.Code")
      src = src:gsub("Enum%.Font%.Gotham", "Enum.Font.Code")
      src = src:gsub("Enum%.Font%.SourceSansBold", "Enum.Font.Code")
      src = src:gsub("Enum%.Font%.SourceSans", "Enum.Font.Code")
      src = src:gsub("Enum%.Font%.ArialBold", "Enum.Font.Code")
      src = src:gsub("Enum%.Font%.Arial", "Enum.Font.Code")
...
do
  ...
  pcall(function()
    ...
    if pg then
      local old2 = pg:FindFirstChild("NyannMinimizeIcon")
  ...
  local busy = false
  local function ToggleWithSpin()
    if busy then return end
    busy = true
    ToggleXeziosMenu()
    pcall(function()
      local goal = btn.Rotation + (MenuVisible and 360 or -360)
...
-- STYLE: Font Code + rounded corners
task.spawn(function()
  local function style(obj)
    ...
    pcall(function()
      ...
      if obj:IsA("UICorner") then
        local px = obj.CornerRadius.Offset or 0
        if px < 14 then
          obj.CornerRadius = UDim.new(0, math.max(px + 4, 12))
        end
      end
    end)
  end
  local function walk(root)
    ...
    pcall(function()
      style(root)
      for _, d in ipairs(root:GetDescendants()) do style(d) end
      root.DescendantAdded:Connect(function(d) task.defer(style, d) end)
    end)
  end
  local function scan()
    pcall(function()
      for _, g in ipairs(game:GetService("CoreGui"):GetChildren()) do
        if g:IsA("ScreenGui") then walk(g) end
      end
    end)
    pcall(function()
  ...
  scan()
  pcall(function()
    if type(ApplyGlobalFont) == "function" then ApplyGlobalFont(Enum.Font.Code) end
  end)
  print("[nyann os] style Font.Code + corner applied")
end)
