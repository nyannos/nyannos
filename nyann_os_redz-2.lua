...
-- ========================================
-- ========================================
-- REDZ V5 REMAKE + minimize kiểu nyann os xova
-- Lib: tlredz/Library redz-V5-remake
-- Xoá minimizer gốc của redz, dùng icon 94678517792779
-- ========================================

local redz = loadstring(game:HttpGet(
...
-- Menu to hơn (Redz default 550x380)
pcall(function()
	if redz.Default then
		redz.Default.UISize = UDim2.fromOffset(640, 450)
		redz.Default.TabSize = 180
	end
	if redz.Save then
		redz.Save.UISize = UDim2.fromOffset(640, 450)
		redz.Save.TabSize = 180
	end
end)

local RealWindow = redz:MakeWindow({
	Title = "nyann os",
	SubTitle = "by real_@nyannnokonoko",
	ScriptFolder = "nyannOS_Redz",
})

-- Ép size MainFrame to hơn (phòng config cũ)
task.defer(function()
	pcall(function()
		local els = RealWindow:GetElements()
		if els and els.MainFrame then
			els.MainFrame.Size = UDim2.fromOffset(640, 450)
		end
	end)
	-- KHÔNG xoá nút "-" gốc — để MinimizeButton thu menu
end)

-- Compatibility shim Tabs.* → Redz
local Window = {}
...
local function MakeTabShim(page)
	...
	function tab:AddSection(name)
		pcall(function()
			page:AddSection(tostring(name or "Section"))
		end)
		return page
	end
	...
	function tab:AddButton(cfg)
		...
		return page:AddButton({
			...
			Title = cfg.Name or cfg.Title or "Button",
			Callback = cfg.Callback or cfg.Func or function() end,
		})
	end

	function tab:AddDropdown(cfg)
		cfg = cfg or {}
		local values = cfg.Options or cfg.Values or cfg.Items or {}
		local def = cfg.Default
		-- Redz Default: string / table
		if type(def) == "number" and values[def] then
			def = values[def]
		end
	...
	function tab:AddSlider(cfg)
		...
		local inc = 1
		if cfg.Rounding and cfg.Rounding > 0 then
			inc = 10 ^ (-cfg.Rounding)
		elseif cfg.Increment then
			inc = cfg.Increment
		end
	...
	function tab:AddTextBox(cfg)
		cfg = cfg or {}
		return page:AddTextBox({
			Name = cfg.Name or cfg.Title or "Input",
			Title = cfg.Name or cfg.Title or "Input",
	...
	function tab:AddParagraph(a, b)
		local para
		pcall(function()
			para = page:AddParagraph(tostring(a or ""), tostring(b or ""))
		end)
		local proxy = { _para = para }
		function proxy:SetDesc(value)
			pcall(function()
				if para and para.SetDescription then
					para:SetDescription(tostring(value or ""))
				elseif para and para.SetDesc then
					para:SetDesc(tostring(value or ""))
				end
			end)
		end
		function proxy:SetDescription(value)
			self:SetDesc(value)
		end
		function proxy:Set(value)
			pcall(function()
				if para and para.SetTitle then
					para:SetTitle(tostring(value or ""))
				end
			end)
		end
		return proxy
	...
	function tab:AddDiscordInvite(cfg)
		cfg = cfg or {}
		return page:AddButton({
			Name = cfg.Title or "Discord",
			Title = cfg.Title or "Discord",
			Callback = function()
				if cfg.Invite and setclipboard then
					pcall(setclipboard, tostring(cfg.Invite))
				end
			end,
		})
	end
...
function Window:MakeTab(cfg)
	...
	if type(icon) == "string" and icon:find("rbxassetid") then
		-- redz GetIconByName expects name; strip asset url → use generic
		icon = "home"
	end
	local page = RealWindow:MakeTab({
		Title = title,
		Name = title,
		Icon = icon,
	})
...
function Window:Notify(opts)
	opts = opts or {}
	pcall(function()
		if RealWindow.Notify then
			RealWindow:Notify({
				Title = opts.Title or "nyann os",
				Content = opts.Content or opts.Message or opts.Description or "",
...
print("[nyann os] Redz V5 UI ready")
...
local Tabs = {
	...
	Prehistoric = Window:MakeTab({ Title = "Volcano Event", Icon = "flame" }),
	Esp = Window:MakeTab({ Title = "Stats And Esp", Icon = "eye" }),
	...
	Shop = Window:MakeTab({ Title = "Shop", Icon = "rbxassetid://10734952479" }),
	Misc = Window:MakeTab({ Title = "Miscellaneous", Icon = "settings-2" }),
}

-- ========================================
-- MINIMIZE ICON (kiểu nyann os xova)
-- rbxassetid://94678517792779 — xoá minimizer gốc redz
-- ========================================
do
	...
	pcall(function()
		local old = gethui():FindFirstChild("NyannMinimizeIcon")
		if old then
			old:Destroy()
		end
	end)
	pcall(function()
		...
		if pg and pg:FindFirstChild("NyannMinimizeIcon") then
			pg.NyannMinimizeIcon:Destroy()
		end
	end)

	-- Nút icon xova: thu/mở menu (giống nút "-" Redz)
	local function toggleMenu()
		pcall(function()
			-- MinimizeButton = thu còn title bar (nút -)
			if RealWindow and RealWindow.MinimizeButton then
				RealWindow:MinimizeButton()
				return
			end
			-- fallback: ẩn/hiện cả cửa sổ
			if RealWindow and RealWindow.Minimize then
				RealWindow:Minimize()
			end
		end)
	end
	...
	btn.Position = UDim2.new(0.02, 0, 0.12, 0)
	...
	btn.MouseButton1Click:Connect(function()
		...
		if clickPos and (now - clickPos).Magnitude > 14 then
			return
		end
		pcall(function()
			TS:Create(btn, TweenInfo.new(0.25, Enum.EasingStyle.Quad), {
				Rotation = btn.Rotation + 360,
			}):Play()
		end)
		toggleMenu()
	end)
	...
	getgenv().NyannToggleMenu = toggleMenu
	print("[nyann os] Minimize xova-style ready")
end
...
pcall(function() ApplyGlobalFont(Enum.Font.Code) end)
