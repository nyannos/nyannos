...
-- ========================================
-- ========================================
-- UiRedzV5 (daucobonhi) + AddMinimizeButton xova icon
-- Không đổi kích thước menu (giữ default lib)
-- ========================================
...
-- Minimize kiểu xova (icon gốc 94678517792779)
pcall(function()
...
-- Compatibility shim: Tabs.* → UiRedzV5
local Window = {}
...
local function MakeTabShim(page)
	...
	function tab:AddToggle(cfg)
		...
		-- hỗ trợ cả table Named và array style
	...
	function tab:AddButton(cfg)
		...
		local cb = cfg.Callback or cfg.Func or function() end
	...
	function tab:AddSlider(cfg)
		...
		local cb = cfg.Callback or function() end
		-- Demo style: {"Slider", min, max, increase, default, callback}
	...
	function tab:AddParagraph(a, b)
		...
		pcall(function()
			if type(a) == "table" then
				para = page:AddParagraph({
					Title = a.Title or a.Name or "",
					Desc = a.Desc or a.Description or a.Content or "",
				})
			else
				para = page:AddParagraph({
					Title = tostring(a or ""),
		...
		function proxy:SetDesc(value)
			pcall(function()
				...
				elseif para and para.Set then
					para:Set(nil, tostring(value or ""))
				end
			end)
		end
...
function Window:MakeTab(cfg)
	...
	local icon = cfg.Icon
	if type(icon) == "string" and (icon:find("rbxassetid") or #icon > 30) then
		icon = "Info"
	end
	-- MakeTab({"Name", "Icon"}) hoặc table
	local page
	pcall(function()
		page = RealWindow:MakeTab({ title, icon or "Info" })
	end)
	if not page then
		pcall(function()
			page = RealWindow:MakeTab({ Name = title, Title = title, Icon = icon })
		end)
	end
...
-- giữ tên redzlib cho đoạn code cũ nếu có gọi
-- (đã load ở trên)

print("[nyann os] UiRedzV5 (daucobonhi) ready")
...
local Tabs = {
	...
	Shop = Window:MakeTab({ Title = "Shop", Icon = "coins" }),
