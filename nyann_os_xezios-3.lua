...
do
  ...
  for _, url in ipairs(urls) do
    local ok, res = pcall(function()
      ...
      src = src:gsub('Font = Enum%.Font%.Arial', 'Font = Enum.Font.Code')
      src = src:gsub('Font = "Gotham"', 'Font = Enum.Font.Code')
...
local RealWindow = Library:Window({
  Prefix = "nyann os by real_@nyannnokonoko",
  Suffix = "",
...
-- ========================================
-- STYLE: Font Code + bo tròn viền menu
-- ========================================
task.spawn(function()
  local function style(obj)
    ...
    pcall(function()
      ...
      if obj:IsA("UICorner") then
        ...
        local px = (type(r) == "UDim" and r.Offset) or 0
        -- bo tròn thêm một chút
        if px < 14 then
          obj.CornerRadius = UDim.new(0, math.max(px + 4, 12))
        end
      elseif obj:IsA("Frame") or obj:IsA("ImageLabel") or obj:IsA("ImageButton") then
        -- frame chính menu: thêm UICorner nếu chưa có
        if obj.Name == "Window" or obj.Name == "Main" or obj.Name == "Background" then
          local corner = obj:FindFirstChildOfClass("UICorner")
          if not corner then
            corner = Instance.new("UICorner")
            corner.CornerRadius = UDim.new(0, 12)
            corner.Parent = obj
          elseif corner.CornerRadius.Offset < 12 then
            corner.CornerRadius = UDim.new(0, 12)
          end
        end
      end
  ...
  task.wait(1.2)
  scan()
  -- auto apply Code qua tab Font nếu có
  pcall(function()
    if ApplyGlobalFont then ApplyGlobalFont(Enum.Font.Code) end
  end)
